...
/**接口中文名**/
public class LogicExampleAction_testRes implements ICodeAble {
	...
	/**测试list注释**/
	private List<Integer> beanList=null;
	/**测试map字段注释**/
	private Map<String,Integer> beanMap=null;
	...
	public void encode(IXOutStream outputStream) throws IOException {
		...
		if(beanList.size()>0){
			for(int beanListi=0;beanListi<beanList.size();beanListi++){
						outputStream.writeInt(beanList.get(beanListi));
		...
		if(beanMap.size()>0){
			for(Entry<String, Integer> entry:beanMap.entrySet()){
...
;
	...
	/**测试list注释**/
    public void setBeanList(List<Integer> beanList) {
		this.beanList = beanList;
	}
	/**测试map字段注释**/
    public Map<String,Integer> getBeanMap() {
		return beanMap;
	}
	/**测试map字段注释**/
    public void setBeanMap(Map<String,Integer> beanMap) {
