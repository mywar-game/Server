...
<html>
	<head>
		...
		<script>
			...
			function changeTypeSelect(categorySelect){
				...
				//获得
				if (category == 1) {
					<s:generator separator="," val="%{getText('userTechnologyPointLog.type_1_values')}">
						html +='<option value="" selected="selected"><s:text name="pleaseSelect"></s:text></option>';
						<s:iterator var="str">
							html +='<option value="${str}"> <s:text name="%{'userTechnologyPointLog.type_1_'+#str}"></s:text> </option>';
						</s:iterator>
					</s:generator>
				//使用
				} else if (category == 2) {
					<s:generator separator="," val="%{getText('userTechnologyPointLog.type_2_values')}">
						html +='<option value="" selected="selected"><s:text name="pleaseSelect"></s:text></option>';
						<s:iterator var="str">
							html +='<option value="${str}"> <s:text name="%{'userTechnologyPointLog.type_2_'+#str}"></s:text> </option>';
						</s:iterator>
					</s:generator>
				//未选择
				} else {
	...
	<body>
		...
		<form action="userTechnologyPointLogList" method="post">
				...
				<tr>
					...
					<td>
						<select id="searchCategory" name="searchCategory" class="select" onchange="changeTypeSelect(this)">
							<s:generator separator="," val="%{getText('userTechnologyPointLog.category_values')}">
								...
								<s:iterator var="str">
									<option value="${str}" <s:if test="searchCategory == #str">selected=selected</s:if>>
										<s:text name="%{'userTechnologyPointLog.category_'+#str}"></s:text>
									</option>
								</s:iterator>
							</s:generator>
						</select>
					...
					<td>
						<select id="searchType" name="searchType" class="select">
							<s:if test="searchCategory != null">
								<option value="" selected="selected"><s:text name="pleaseSelect"></s:text></option>
								<s:generator separator="," val="%{getText('userTechnologyPointLog.type_'+searchCategory+'_values')}">
									<s:iterator var="str">
										<option value="${str}" <s:if test="searchType == #str">selected=selected</s:if>>
											<s:text name="%{'userTechnologyPointLog.type_'+searchCategory+'_'+#str}"></s:text>
										</option>
									</s:iterator>
								</s:generator>
							</s:if>
			...
			<table id="table" cellpadding="3" cellspacing="1" border="0" width="100%" align=center>
				...
				<s:iterator var="log" value="logList" status="sta">
					<tr>
						...
						<td>
							<s:text name="%{'userTechnologyPointLog.category_'+#log.category}"></s:text>
						</td>
						<td>
							<s:text name="%{'userTechnologyPointLog.type_'+#log.category+'_'+#log.type}"></s:text>
						</td>
						<td>
							${log.changeNum}
						</td>
