...
<html>
	<head>
		<title><s:text name="userLevelUpLogListJsp.title"></s:text></title>
	...
	<body>
		<form action="userLevelUpLogList?isCommit=T" method="post" onsubmit="return dateMust()">
			<table>
				<tr>
					<td>
		...
		<table id="table" cellpadding="3" cellspacing="1" border="0" width="100%" align=center >
			<tr class="border">
				<td class="td_title" colspan="7" align="center">
					<center>
						<s:text name="userLevelUpLogListJsp.title"></s:text>
					</center>
				</td>
			</tr>
			<tr>
				<td>
					<s:text name="userLevelupLog.id"></s:text>
				</td>
				<td>
					<s:text name="userLevelupLog.userId"></s:text>
				</td>
				<td>
					<s:text name="userLevelupLog.userName"></s:text>
				</td>
				<td>
					<s:text name="userLevelupLog.lodoId"></s:text>
				</td>
				<td>
					<s:text name="userLevelupLog.level"></s:text>
				</td>
				<td>
					<s:text name="userLevelupLog.time"></s:text>
				</td>
			</tr>
			
			<s:iterator var="userLevelupLog" value="userLevelUpLogList" status="sta">
				<tr>
					<td>${sta.index+1}</td>
					<td>${userLevelupLog.userId}</td>
					<td>${userLevelupLog.userName}</td>
					<td>${userLevelupLog.lodoId}</td>
					<td>${userLevelupLog.level}</td>
					<td>
						<s:text name="format.date_time">
							<s:param value="#userLevelupLog.time"></s:param>
						</s:text>
					</td>
			...
			<tr>
				<td colspan="100">
					<aldtags:pageTag paraStr="isCommit,T,userId,${userId},lodoId,${lodoId},userName,${userName},name,${name}" datePara1="startDate" dateValue1="${startDate}" datePara2="endDate" dateValue2="${endDate}"/>
				</td>
			</tr>
		</table>
	</body>
