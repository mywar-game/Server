...
<html>
	<head>
		<title><s:text name="userFriendVisitStatsListJsp.title"></s:text></title>
		<style type="text/css">
		td {
			text-align: center;
		}
	...
	<script type="text/javascript"> 
		...
		//			max : arg.yAxis==undefined?null:(arg.yAxis.max==undefined?null:arg.yAxis.max)
				...
				tooltip: {
					formatter: function() {
						return '<b>' + this.series.name + '</b><br/>' + "<s:text name="userFriendVisitStats.date"></s:text><s:text name="colon"></s:text>" + this.x +"    <s:text name="stats.num"></s:text><s:text name="colon"></s:text> " + this.y;
					}
				},
				legend: {
					layout: 'vertical',
	...
	<body>
		<form action="userFriendVisitStatsList" method="post" onsubmit="return check()">
		...
		<table id="table" cellpadding="3" cellspacing="1" border="0" width="100%" align=center>
			<tr class="border">
				<td class="td_title" colspan="22" align="center">
					<center>
						<s:text name="userFriendVisitStatsListJsp.title"></s:text>
					</center>
				</td>
			</tr>
			<tr>
				<td>
					<s:text name="userFriendVisitStats.date"></s:text>
				</td>
				<td>
					<s:text name="userFriendVisitStats.visitTimes"></s:text>
				</td>
				<td>
					<s:text name="userFriendVisitStats.averageVisitTimes"></s:text>
				</td>
				<td>
					<s:text name="userFriendVisitStats.visitPeak"></s:text>
				</td>
			</tr>
			<s:iterator var="stats" value="statsList">
				<tr>
					...
					<td>
						<fmt:formatNumber type="number" value="${stats.visitTimes/stats.visitUserNum}" pattern="#,###" />
					</td>
					<td>
						${stats.visitPeak}
					</td>
