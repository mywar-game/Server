...
<html>
	<head>
		<title><s:text name="editmenuchildJsp.title"></s:text></title>
	...
	<body>
		<form action="editmenuchild?isCommit=T&id=${id}" method="post" name="pform" onsubmit="return checkform();">
			<table cellpadding="3" cellspacing="1" border="0" width="100%" align=center>
				<tr class="border">
					<td class="td_title" colspan="3">
						<s:text name="editmenuchildJsp.title"></s:text> &nbsp;
					...
					<td>
						<select name="orderId" class="select">
							<s:generator separator="," val="%{getText('adminMenu.orderId_value')}">
								<s:iterator var="str">
									<option value="${str}" <s:if test="orderId == #str">selected=selected</s:if>>
