...
<html>
	<head>
		<title><s:text name="totalDayPayRewardListJsp.title"></s:text></title>
	...
	<script type="text/javascript">
		...
		function addRow() {
			...
			newRow.innerHTML=""
				+ "<td " + cssTd + "><input type='text' size='8' name='dayArr' value='' /></td>" 
				+ "<td " + cssTd + "><input type='text' size='30' readonly='readonly' name='dropToolArr' value='' /></td>"
				+ "<td " + cssTd + "></td>"
	...
	<body>
		<form name="f" action="addTotalDayPayReward?isCommit=T" method="post" >
		<input type="button" value="<s:text name='addTotalDayPayReward.title'></s:text>" class="button" onclick='addRow()'; />
		<table id="toolTable" cellpadding="3" cellspacing="1" border="0" width="100%" align=center>
			<tr class="border">
				<td class="td_title" colspan="22" align="center">
					<center>
						<s:text name="totalDayPayRewardListJsp.title"></s:text>
					</center>
				</td>
			</tr>
			<tr>
				<td >
					...
					<s:text name="totalDayPayReward.day"></s:text>
				</td>
				<td>
					<s:text name="totalDayPayReward.dropTool"></s:text>(ids)
				</td>
				<td>
					<s:text name="totalDayPayReward.dropTool"></s:text>
				</td>
				<td>
			...
			<s:iterator var="totalDay" value="totalDayPayRewardList" >
			<tr id="${totalDay.id}" >
				<%-- <td>
					<input type="text" size="4" name="idArr" value="${totalDay.id}" />
				</td> --%>
				<td >
					<input type="text" size="8" name="dayArr" value="${totalDay.day}" />								
				</td>
				<td >
					<input type="text" size="30" readonly="readonly" name="dropToolArr" value="${totalDay.dropTool}" />								
				</td>
				<td >					
					${totalDay.showDropTool}
				</td>
				<td >
					<input type="text" size="50" name="descriptionArr" value="${totalDay.toolDesc}" />
				</td>				
				<td>
