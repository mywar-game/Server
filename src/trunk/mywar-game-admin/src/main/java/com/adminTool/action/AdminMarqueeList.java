 ...
 * @author lin
 */
public class AdminMarqueeList extends ALDAdminPageActionSupport {
	...
	public String execute() {
		...
		for (AdminMarquee marquee : marqueeList) {
			...
			if(marquee.getPartnerIds().trim().equals("0"))
				marquee.setPartnerNames("全渠道");
			else if(!partnerMap.containsKey(marquee.getPartnerIds()))
				marquee.setPartnerNames("未知渠道");
			else
				marquee.setPartnerNames(partnerMap.get(marquee.getPartnerIds()).getPName());
		}
