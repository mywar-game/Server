 ...
 *
 */
public class AllServerInfoListNew extends ALDAdminStatsDatePageActionSupport {
...
//				sheet.addCell(label14);
	...
	public String findResult(boolean isExport){
		...
		if (isExport || isGrap.equals("T")) {//导出时查出全部数据
			pageList = homePageService.findDataNew(dates, sysNum, channel, qn);
		} else {//不是导出则分页查询
			IPage<Object> page = homePageService.findPageDataNew(dates, channel, qn, super.getPageSize(), super.getToPage());
			if (page != null) {
...
//				}
				...
				if (arr[11].toString() == null || partnerMap.get(arr[11].toString()) == null) {
					map.put("channelName", "未命名渠道号" + arr[11].toString());
				} else {
					String channelName = partnerMap.get(arr[11].toString()).getPName();
