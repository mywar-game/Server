 ...
 *
 */
public class RePayIntervalStatsCollector {

	public void execute() {
		LogSystem.info("手动--充值区间统计Collector开始");
		...
		if (statsMap != null && statsMap.size() > 0) {
			...
			for (Entry<Long, Integer> entry : entrySet) {
				...
				String key = payIntervalIndex*100+"-"+(payIntervalIndex + 1)*100;
		...
		LogSystem.info("手动--充值区间统计Collector完毕");
	}
}
