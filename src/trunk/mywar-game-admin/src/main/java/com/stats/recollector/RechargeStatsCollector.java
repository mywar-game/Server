...
public class RechargeStatsCollector {

	public void execute(String dateStr) {
		
		LogSystem.info("手动--充值统计开始-------------");
...
//		Date date = DateUtil.getSomeDaysDiffDate(SystemStatsDate.YESTERDAY);
		...
		// 先删除数据
		rechargeStatsService.delete(date);
		
		List<Object> payList = userPayService.findTotalPayUserNumAndTotalAmount(dates, true); // 以渠道分组的付费总用户和付费总额
		List<Object> newPayList = userPayService.findNewUserPayNumAndPayAmount2(dates, true);  // 以渠道分组的新用户付费数和新用户付费总额
