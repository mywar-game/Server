 ...
 *         2011-10-26
 */
public class MonthCollectorEntry extends SchedulerEntry {

	@Override
	public void executeJob(JobExecutionContext jobExecutionContext) throws JobExecutionException {
		LogSystem.info("月统计开始"+jobExecutionContext.getJobDetail().getName());
		...
		for (int i = 0; i < serverIdsArr.length; i++) {
			...
			InvocationCollector invocationCollector = (InvocationCollector) ServiceCacheFactory .getServiceCache().getBeanById("monthInvocationCollector");
			invocationCollector.invoke(date);
			LogSystem.info("月统计完毕 "+jobExecutionContext.getJobDetail().getName());
