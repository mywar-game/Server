...
public class UserRemainStatsByIdfaCollector implements Collector {
	...
	public void execute(Date time) throws Exception {
		LogSystem.info("用户留存统计Collector开始(根据idfa)");
		...
		Map<String, UserRemainByIdfaStats> statsMap = statsService.findNowServerData();
		...
		UserRemainByIdfaStats yesterdayStats = new UserRemainByIdfaStats();
//		int userCount = userService.countAllUser(); // 用户总量
		int userCount = userService.countAllUserByIdfa(); // 根据idfa统计创角总数
		
//		List<Object> newRegList = userService.findCreateUserNumInSomeTime(dates, false);// 新注册数, 原来那个有问题.
		int newReg = userRegLogService.countSomeDayRegedUserByIdfa(SystemStatsDate.YESTERDAY);
		int newCreated = userRegLogService.countSomeDayCreatedUserByIdfa(SystemStatsDate.YESTERDAY);
		
//		List<Object> loginList = userLoginLogService.findActiveUserAmount(dates, false);
		
		List<Object> loginList = userLoginLogService.findActiveUserAmountByIdfa(dates);
...
//		}
		yesterdayStats.setNewReg(newReg); // 新注册，不是新创角
		yesterdayStats.setNewCreated(newCreated);
		...
		UserRemainByIdfaStats userRemainStats;
		// 统计2天前那天的的次日留存
		if (statsMap.containsKey(DateUtil.getTime(-ONE_DAY_REMAIN_DAYS))) {
			...
			if (userRemainStats.getOneDayRemain() == null) {
				...
				try {
					int remainUsers = userLoginLogService.findUserRemainInfoByIdfa(dates, dates1);
					// 2天前的那天注册的人数
					// int regUserNum = userService.countSomeDayRegedUser(ONE_DAY_REMAIN_DAYS);
					int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-ONE_DAY_REMAIN_DAYS);
		...
		if (statsMap.containsKey(DateUtil.getTime(-TWO_DAY_REMAIN_DAYS))) {
			...
			if (userRemainStats.getTwoDayRemain() == null) {
				...
				try {
					...
					int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-TWO_DAY_REMAIN_DAYS);
		...
		if (statsMap.containsKey(DateUtil.getTime(-THREE_DAY_REMAIN_DAYS))) {
			...
			if (userRemainStats.getThreeDayRemain() == null) {
				...
				try {
					...
					int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-THREE_DAY_REMAIN_DAYS);
		...
		if (statsMap.containsKey(DateUtil.getTime(-FOUR_DAY_REMAIN_DAYS))) {
			...
			if (userRemainStats.getFourDayRemain() == null) {
				...
				try {
					...
					int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-FOUR_DAY_REMAIN_DAYS);
		...
		if (statsMap.containsKey(DateUtil.getTime(-FIVE_DAY_REMAIN_DAYS))) {
			...
			if (userRemainStats.getFiveDayRemain() == null) {
				...
				try {
					...
					int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-FIVE_DAY_REMAIN_DAYS);
		...
		if (statsMap.containsKey(DateUtil.getTime(-SIX_DAY_REMAIN_DAYS))) {
			...
			if (userRemainStats.getSixDayRemain() == null) {
				...
				try {
					...
					int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-SIX_DAY_REMAIN_DAYS);
		...
		if (statsMap.containsKey(DateUtil.getTime(-SEVEN_DAY_REMAIN_DAYS))) {
			...
			if (userRemainStats.getSevenDayRemain() == null) {
				...
				try {
					...
					int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-SEVEN_DAY_REMAIN_DAYS);
				...
				if (statsMap.containsKey(DateUtil.getTime(-EIGHT_DAY_REMAIN_DAYS))) {
					...
					if (userRemainStats.getEightDayRemain() == null) {
						...
						try {
							...
							// 9天前的那天注册的人数
							// int regUserNum = userService.countSomeDayRegedUser(SEVEN_DAY_REMAIN_DAYS);
							int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-EIGHT_DAY_REMAIN_DAYS);
				...
				if (statsMap.containsKey(DateUtil.getTime(-NIGHT_DAY_REMAIN_DAYS))) {
					...
					if (userRemainStats.getNeightDayRemain() == null) {
						...
						try {
							...
							// 10天前的那天注册的人数
							// int regUserNum = userService.countSomeDayRegedUser(SEVEN_DAY_REMAIN_DAYS);
							int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-NIGHT_DAY_REMAIN_DAYS);
				...
				if (statsMap.containsKey(DateUtil.getTime(-TEN_DAY_REMAIN_DAYS))) {
					...
					if (userRemainStats.getTenDayRemain() == null) {
						...
						try {
							...
							// 11天前的那天注册的人数
							// int regUserNum = userService.countSomeDayRegedUser(SEVEN_DAY_REMAIN_DAYS);
							int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-TEN_DAY_REMAIN_DAYS);
				...
				if (statsMap.containsKey(DateUtil.getTime(-ELEVEN_DAY_REMAIN_DAYS))) {
					...
					if (userRemainStats.getElevenDayRemain() == null) {
						...
						try {
							...
							// 12天前的那天注册的人数
							// int regUserNum = userService.countSomeDayRegedUser(SEVEN_DAY_REMAIN_DAYS);
							int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-ELEVEN_DAY_REMAIN_DAYS);
				...
				if (statsMap.containsKey(DateUtil.getTime(-TWELVE_DAY_REMAIN_DAYS))) {
					...
					if (userRemainStats.getTwelfDayRemain() == null) {
						...
						try {
							...
							// 12天前的那天注册的人数
							// int regUserNum = userService.countSomeDayRegedUser(SEVEN_DAY_REMAIN_DAYS);
							int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-TWELVE_DAY_REMAIN_DAYS);
				...
				if (statsMap.containsKey(DateUtil.getTime(-THIRTEEN_DAY_REMAIN_DAYS))) {
					...
					if (userRemainStats.getThirteenDayRemain() == null) {
						...
						try {
							...
							// 14天前的那天注册的人数
							// int regUserNum = userService.countSomeDayRegedUser(SEVEN_DAY_REMAIN_DAYS);
							int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-THIRTEEN_DAY_REMAIN_DAYS);
				...
				if (statsMap.containsKey(DateUtil.getTime(-FOURTEEN_DAY_REMAIN_DAYS))) {
					...
					if (userRemainStats.getFourteenDayRemain() == null) {
						...
						try {
							...
							// 15天前的那天注册的人数
							// int regUserNum = userService.countSomeDayRegedUser(SEVEN_DAY_REMAIN_DAYS);
							int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-FOURTEEN_DAY_REMAIN_DAYS);
		...
		if (statsMap.containsKey(DateUtil.getTime(-FIFTEEN_DAY_REMAIN_DAYS))) {
			...
			if (userRemainStats.getFifteenDayRemain() == null) {
				...
				try {
					...
					int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-FIFTEEN_DAY_REMAIN_DAYS);
				...
				if (statsMap.containsKey(DateUtil.getTime(-SIXTEEN_DAY_REMAIN_DAYS))) {
					...
					if (userRemainStats.getSixteenDayRemain() == null) {
						...
						try {
							...
							// 17天前的那天注册的人数
							// int regUserNum = userService.countSomeDayRegedUser(FIFTEEN_DAY_REMAIN_DAYS);
							int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-SIXTEEN_DAY_REMAIN_DAYS);
				...
				if (statsMap.containsKey(DateUtil.getTime(-SEVENTEEN_DAY_REMAIN_DAYS))) {
					...
					if (userRemainStats.getSeventeenDayRemain() == null) {
						...
						try {
							...
							// 18天前的那天注册的人数
							// int regUserNum = userService.countSomeDayRegedUser(FIFTEEN_DAY_REMAIN_DAYS);
							int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-SEVENTEEN_DAY_REMAIN_DAYS);
				...
				if (statsMap.containsKey(DateUtil.getTime(-EIGHTTEEN_DAY_REMAIN_DAYS))) {
					...
					if (userRemainStats.getEightteenDayRemain() == null) {
						...
						try {
							...
							// 19天前的那天注册的人数
							// int regUserNum = userService.countSomeDayRegedUser(FIFTEEN_DAY_REMAIN_DAYS);
							int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-EIGHTTEEN_DAY_REMAIN_DAYS);
				...
				if (statsMap.containsKey(DateUtil.getTime(-NIGHTTEEN_DAY_REMAIN_DAYS))) {
					...
					if (userRemainStats.getNeightteenDayRemain() == null) {
						...
						try {
							...
							// 20天前的那天注册的人数
							// int regUserNum = userService.countSomeDayRegedUser(FIFTEEN_DAY_REMAIN_DAYS);
							int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-NIGHTTEEN_DAY_REMAIN_DAYS);
				...
				if (statsMap.containsKey(DateUtil.getTime(-TWENTIE_DAY_REMAIN_DAYS))) {
					...
					if (userRemainStats.getTwentieDayRemain() == null) {
						...
						try {
							...
							// 21天前的那天注册的人数
							// int regUserNum = userService.countSomeDayRegedUser(FIFTEEN_DAY_REMAIN_DAYS);
							int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-TWENTIE_DAY_REMAIN_DAYS);
				...
				if (statsMap.containsKey(DateUtil.getTime(-TWENTIEONE_DAY_REMAIN_DAYS))) {
					...
					if (userRemainStats.getTwentieoneDayRemain() == null) {
						...
						try {
							...
							// 22天前的那天注册的人数
							// int regUserNum = userService.countSomeDayRegedUser(FIFTEEN_DAY_REMAIN_DAYS);
							int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-TWENTIEONE_DAY_REMAIN_DAYS);
				...
				if (statsMap.containsKey(DateUtil.getTime(-TWENTIETWO_DAY_REMAIN_DAYS))) {
					...
					if (userRemainStats.getTwentietwoDayRemain() == null) {
						...
						try {
							...
							// 23天前的那天注册的人数
							// int regUserNum = userService.countSomeDayRegedUser(FIFTEEN_DAY_REMAIN_DAYS);
							int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-TWENTIETWO_DAY_REMAIN_DAYS);
				...
				if (statsMap.containsKey(DateUtil.getTime(-TWENTIETHREE_DAY_REMAIN_DAYS))) {
					...
					if (userRemainStats.getTwentiethreeDayRemain() == null) {
						...
						try {
							...
							// 24天前的那天注册的人数
							// int regUserNum = userService.countSomeDayRegedUser(FIFTEEN_DAY_REMAIN_DAYS);
							int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-TWENTIETHREE_DAY_REMAIN_DAYS);
				...
				if (statsMap.containsKey(DateUtil.getTime(-TWENTIEFOUR_DAY_REMAIN_DAYS))) {
					...
					if (userRemainStats.getTwentiefourDayRemain() == null) {
						...
						try {
							...
							// 24天前的那天注册的人数
							// int regUserNum = userService.countSomeDayRegedUser(FIFTEEN_DAY_REMAIN_DAYS);
							int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-TWENTIEFOUR_DAY_REMAIN_DAYS);
				...
				if (statsMap.containsKey(DateUtil.getTime(-TWENTIEFIVE_DAY_REMAIN_DAYS))) {
					...
					if (userRemainStats.getTwentiefiveDayRemain() == null) {
						...
						try {
							...
							// 26天前的那天注册的人数
							// int regUserNum = userService.countSomeDayRegedUser(FIFTEEN_DAY_REMAIN_DAYS);
							int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-TWENTIEFIVE_DAY_REMAIN_DAYS);
				...
				if (statsMap.containsKey(DateUtil.getTime(-TWENTIESIX_DAY_REMAIN_DAYS))) {
					...
					if (userRemainStats.getTwentiesixDayRemain() == null) {
						...
						try {
							...
							// 27天前的那天注册的人数
							// int regUserNum = userService.countSomeDayRegedUser(FIFTEEN_DAY_REMAIN_DAYS);
							int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-TWENTIESIX_DAY_REMAIN_DAYS);
				...
				if (statsMap.containsKey(DateUtil.getTime(-TWENTIESEVEN_DAY_REMAIN_DAYS))) {
					...
					if (userRemainStats.getTwentiesevenDayRemain() == null) {
						...
						try {
							...
							// 28天前的那天注册的人数
							// int regUserNum = userService.countSomeDayRegedUser(FIFTEEN_DAY_REMAIN_DAYS);
							int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-TWENTIESEVEN_DAY_REMAIN_DAYS);
				...
				if (statsMap.containsKey(DateUtil.getTime(-TWENTIEEIGHT_DAY_REMAIN_DAYS))) {
					...
					if (userRemainStats.getTwentieeightDayRemain() == null) {
						...
						try {
							...
							// 29天前的那天注册的人数
							// int regUserNum = userService.countSomeDayRegedUser(FIFTEEN_DAY_REMAIN_DAYS);
							int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-TWENTIEEIGHT_DAY_REMAIN_DAYS);
				...
				if (statsMap.containsKey(DateUtil.getTime(-TWENTIENEIGHT_DAY_REMAIN_DAYS))) {
					...
					if (userRemainStats.getTwentienightDayRemain() == null) {
						...
						try {
							...
							// 30天前的那天注册的人数
							// int regUserNum = userService.countSomeDayRegedUser(FIFTEEN_DAY_REMAIN_DAYS);
							int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-TWENTIENEIGHT_DAY_REMAIN_DAYS);
		...
		if (statsMap.containsKey(DateUtil.getTime(-THIRTY_DAY_REMAIN_DAYS))) {
			...
			if (userRemainStats.getThirtyDayRemain() == null) {
				...
				try {
					...
					int regUserNum = userRegLogService.countSomeDayRegedUserByIdfa(-THIRTY_DAY_REMAIN_DAYS);
		...
		LogSystem.info("用户留存统计Collector完毕(根据idfa)");
	}
}
