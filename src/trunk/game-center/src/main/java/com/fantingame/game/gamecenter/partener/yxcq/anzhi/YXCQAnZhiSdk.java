...
public class YXCQAnZhiSdk extends BaseSdk {

	private static final Logger logger = Logger.getLogger(YXCQAnZhiSdk.class);

	private static YXCQAnZhiSdk ins;
	...
	public static YXCQAnZhiSdk instance() {
		synchronized (YXCQAnZhiSdk.class) {
			if (ins == null) {
				ins = new YXCQAnZhiSdk();
			}
		}
	...
	public boolean verifySession(String session, String uid, long timestamp) {
		...
		params.put("sign",
				Base64.encodeToString(apiKey + uid + session + appSecret));
		...
		if (jsonMap.containsKey("sc") && jsonMap.get("sc") != null
	...
	public boolean checkPayCallbackSign(AnZhiPaymentObj data) {
		try {
			String jsonStr = Des3Util.decrypt(data.getData(), YXCQAnZhiSdk
					.instance().getAppSecret());
			logger.info("anzhi checkPayCallback :" + jsonStr);
