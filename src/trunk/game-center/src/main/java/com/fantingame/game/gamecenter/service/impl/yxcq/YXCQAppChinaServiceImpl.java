...
import com.fantingame.game.gamecenter.partener.yxcq.appchina.AppChinaPaymentObj;
import com.fantingame.game.gamecenter.partener.yxcq.appchina.YXCQAppChinaSdk;
...
public class YXCQAppChinaServiceImpl extends BasePartnerService {
	private static final Logger logger = Logger.getLogger(YXCQAppChinaServiceImpl.class);
	@Autowired
	...
	public UserToken login(String token, String partnerId, String serverId, long timestamp, String sign, Map<String, String> params) {
		...
		try {
			...
			if (YXCQAppChinaSdk.instance().verifySession(session, uid)) {
	...
	public boolean doPayment(PaymentObj paymentObj) {
		...
		AppChinaPaymentObj cb = (AppChinaPaymentObj) paymentObj;
		if (!YXCQAppChinaSdk.instance().checkPayCallbackSign(cb)) {
			logger.error("签名不正确" + Json.toJson(paymentObj));
			return false;
		}
		logger.info("game id:" + cb.getExorderno());
		...
		if (this.paymentOrderDao.updateStatus(order.getOrderId(), OrderStatus.STATUS_FINISH, cb.getTransid(), finishAmount, "")) {
			...
			if (!GameApiSdk.getInstance().doPayment(order.getPartnerId(), order.getServerId(), order.getPartnerUserId(), "", order.getOrderId(), finishAmount, gold, "", "",gameServer)) {
				// 如果失败，要把订单置为未支付
				this.paymentOrderDao.updateStatus(order.getOrderId(), OrderStatus.STATUS_NOT_PAY, cb.getTransid(), finishAmount, "");
	...
	public PartenerEnum getPatenerEnum() {
		return PartenerEnum.YXCQ_AppChina;
	}
