...
import com.fantingame.game.gamecenter.partener.yxcq.apple.YXCQAppleSdk;
...
public class YXCQAppleServiceImpl extends BasePartnerService {
	private static final Logger logger = Logger.getLogger(YXCQAppleServiceImpl.class);
	@Autowired
	...
	public boolean doPayment(PaymentObj paymentObj) {
		...
		int n = 0;
		while (!YXCQAppleSdk.instance().checkPayCallbackSign(cb)) {
			if (n >= 5) {
	...
	public PartenerEnum getPatenerEnum() {
		return PartenerEnum.YXCQ_AppStore;
	}
}
