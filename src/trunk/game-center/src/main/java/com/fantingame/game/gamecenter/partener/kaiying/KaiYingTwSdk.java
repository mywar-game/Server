...
public class KaiYingTwSdk extends BaseSdk {
	
	private static final Logger logger = Logger.getLogger(KaiYingTwSdk.class);

	private static KaiYingTwSdk ins;
	...
	public static KaiYingTwSdk instance() {
		synchronized (KaiYingTwSdk.class) {
			if (ins == null) {
				ins = new KaiYingTwSdk();
			}
		}
	...
	private void loadSdkProperties() {
		try {
			prop = PropertiesLoaderUtils.loadProperties(new ClassPathResource(
					"sdk.properties"));
			resource = prop.getProperty("KaiYingTwSdk.resource");
			key = prop.getProperty("KaiYingTwSdk.key");
			partnerId = prop.getProperty("KaiYingTwSdk.partnerId");
		} catch (IOException e) {
	...
	public boolean checkPayCallbackSign(KaiYingTwPaymentObj cb) {
		...
		try {
			String signString = KaiYingTwSdk.instance().getKey() + cb.getUserId() + cb.getSid() + cb.getNumber() + 
					cb.getAmount() + cb.getRoleId() + cb.getPartnerOrderId() + cb.getTs() + KaiYingTwSdk.instance().getResource();
