...
import com.fantingame.game.gamecenter.partener.yxcq.amigo.AmigoPaymentObj;
import com.fantingame.game.gamecenter.partener.yxcq.amigo.YXCQAmigoSdk;
 ...
 * @author yezp
 */
public class YXCQAmigoServiceImpl extends BasePartnerService {
	
	private static final Logger logger = Logger.getLogger(YXCQAmigoServiceImpl.class);
	@Autowired
	...
	public UserToken login(String token, String partnerId, String serverId,
		...
		try {
			...
			String strJson = YXCQAmigoSdk.instance().verify(token);
	...
	public boolean doPayment(PaymentObj paymentObj) {
		...
		if (!YXCQAmigoSdk.instance().checkPayCallbackSign(cb)) {
	...
	public String createOrder(String partnerId, String serverId,
		...
		TradeInfo info = createOrderInfo(partnerId, serverId, partnerUserId,
				amount, tradeName);
//		info.setNotifyUrl("http://wapi.andr.hw.fantingame.com:80/webApi/yxcqamigoPayment.do");
		info.setNotifyUrl("http://203.195.190.121:8089/webApi/yxcqamigoPayment.do");
...
//		info.setNotifyUrl("http://wapi.andr.hw.fantingame.com:80/webApi/yxcqamigoPayment.do");
	...
	private void createServerOrder(TradeInfo info) {
		YXCQAmigoSdk.instance().createServerOrder(info);
	}
	
	@Override
	public PartenerEnum getPatenerEnum() {
		return PartenerEnum.YXCQ_Amigo;
	}
}
