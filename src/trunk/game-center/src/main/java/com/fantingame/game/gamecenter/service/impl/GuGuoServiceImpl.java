...
public class GuGuoServiceImpl extends BasePartnerService {
	private static final Logger logger = Logger.getLogger(GuGuoServiceImpl.class);
	@Autowired
	...
	public UserToken login(String token, String partnerId, String serverId, long timestamp, String sign, Map<String, String> params) {
		...
		try {
			...
			String loginTimesamp = tokenArr[1];
			String session = tokenArr[2];
			if (GuGuoSdk.instance().verifySession(session, uid, loginTimesamp)) {
	...
	public boolean doPayment(PaymentObj paymentObj) {
		...
		GuGuoPaymentObj cb = (GuGuoPaymentObj) paymentObj;
		if (!GuGuoSdk.instance().checkPayCallbackSign(cb)) {
			logger.error("签名不正确" + Json.toJson(paymentObj));
			return false;
		}
		logger.info("game id:" + cb.getCallBackInfo());
		PaymentOrder order = paymentOrderDao.get(cb.getCallBackInfo());
		...
		if (!cb.getOrderStatus().equals("1")) {
			logger.error("充值失败：" + Json.toJson(cb));
			this.paymentOrderDao.updateStatus(order.getOrderId(), OrderStatus.STATUS_ERROR, cb.getOrderId(), finishAmount, cb.getRemark());
			return false;
		}

		int gold = (int) (order.getAmount().doubleValue() * 10);
		// 更新订单状态
		if (this.paymentOrderDao.updateStatus(order.getOrderId(), OrderStatus.STATUS_FINISH, cb.getOrderId(), finishAmount, cb.getRemark())) {
			GameServer gameServer = serverListDao.getServerByServerIdAndPartenerId(order.getServerId(), order.getPartnerId());
			// 请求游戏服，发放游戏货币
			if (!GameApiSdk.getInstance().doPayment(order.getPartnerId(), order.getServerId(), order.getPartnerUserId(), cb.getRemark(), order.getOrderId(), finishAmount, gold, "", "",gameServer)) {
				// 如果失败，要把订单置为未支付
				this.paymentOrderDao.updateStatus(order.getOrderId(), OrderStatus.STATUS_NOT_PAY, cb.getOrderId(), finishAmount, cb.getRemark());
