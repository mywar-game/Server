...
public class ShiJiaServiceImpl extends BasePartnerService {
	private static final Logger logger = Logger.getLogger(ShiJiaServiceImpl.class);
	@Autowired
	...
	public UserToken login(String token, String partnerId, String serverId, long timestamp, String sign, Map<String, String> params) {
		...
		try {
			...
			String loginTime = tokenArr[2];
			if (ShiJiaSdk.instance().verifySession(session, uid, loginTime)) {
	...
	public boolean doPayment(PaymentObj paymentObj) {
		...
		ShiJiaPaymentObj cb = (ShiJiaPaymentObj) paymentObj;

		if (!ShiJiaSdk.instance().checkPayCallbackSign(cb)) {
		...
		// OrderStatus.STATUS_ERROR, cb.getOrderId(),
		// finishAmount,cb.getMemo());
		// return false;
		// }

		int gold = (int) (order.getAmount().doubleValue() * 10);
		// 更新订单状态
		if (this.paymentOrderDao.updateStatus(order.getOrderId(), OrderStatus.STATUS_FINISH, cb.getBillno(), finishAmount, "")) {
			...
			if (!GameApiSdk.getInstance().doPayment(order.getPartnerId(), order.getServerId(), order.getPartnerUserId(), "", order.getOrderId(), finishAmount, gold, "", "",gameServer)) {
				// 如果失败，要把订单置为未支付
				this.paymentOrderDao.updateStatus(order.getOrderId(), OrderStatus.STATUS_NOT_PAY, cb.getBillno(), finishAmount, "");
