...
import com.fantingame.game.gamecenter.partener.yxcq.anzhi.AnZhiPaymentObj;
import com.fantingame.game.gamecenter.partener.yxcq.anzhi.YXCQAnZhiSdk;
...
public class YXCQAnZhiServiceImpl extends BasePartnerService {
	private static final Logger logger = Logger.getLogger(YXCQAnZhiServiceImpl.class);
	@Autowired
	...
	public UserToken login(String token, String partnerId, String serverId, long timestamp, String sign, Map<String, String> params) {
		...
		try {
			...
			if (YXCQAnZhiSdk.instance().verifySession(session, uid, timestamp)) {
	...
	public boolean doPayment(PaymentObj paymentObj) {
		...
		if (!YXCQAnZhiSdk.instance().checkPayCallbackSign(cb)) {
...
//		}
	...
	public PartenerEnum getPatenerEnum() {
		return PartenerEnum.YXCQ_AnZhi;
	}
