 ...
 */
@Controller
public class YXCQHuaWeiController {
	private static Logger LOG = Logger.getLogger(YXCQHuaWeiController.class);

	@Autowired
	private PartnerServiceFactory serviceFactory;

	@RequestMapping(value = "/webApi/yxcqhuaWeiPayment.do")
	public ModelAndView payCallback(HttpServletRequest req,
			HttpServletResponse res) {
		PartnerService ps = serviceFactory.getBean(PartenerEnum.YXCQ_huaWei
				.getPartenerId());
		try {
			...
			if (StringUtils.isNotBlank(orderId)
				...
				if (!YXCQHuaWeiSdk.instance().checkPayCallbackSign(data)) {
