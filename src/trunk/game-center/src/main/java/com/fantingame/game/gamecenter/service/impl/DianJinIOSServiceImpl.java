...
import com.fantingame.game.gamecenter.partener.dianjin.DianJinIOSSdk;
...
public class DianJinIOSServiceImpl extends BasePartnerService {

	public static Logger logger = Logger.getLogger(DianJinIOSServiceImpl.class);
	...
	public UserToken login(String token, String partnerId, String serverId, long timestamp, String sign, Map<String, String> params) {
		...
		try {
			...
			// token:uid
			if(DianJinIOSSdk.instance().verifySession(paras[0],paras[1])){
				GameServer gameServer = serverListDao.getServerByServerIdAndPartenerId(serverId, partnerId);
				return GameApiSdk.getInstance().loadUserToken(paras[1], partnerId, serverId,"0",gameServer, params);
			}
		} catch (Exception e) {
	...
	public boolean doPayment(PaymentObj paymentObj) {
		...
		if(!DianJinIOSSdk.instance().checkPayCallbackSign(cb)) {
	...
	public PartenerEnum getPatenerEnum() {
		return PartenerEnum.DianJinIos;
	}
}
