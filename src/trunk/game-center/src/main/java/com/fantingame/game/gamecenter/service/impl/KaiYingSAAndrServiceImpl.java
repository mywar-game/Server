...
public class KaiYingSAAndrServiceImpl extends BasePartnerService {

	private static final Logger logger = Logger.getLogger(KaiYingSAAndrServiceImpl.class);
	@Autowired
...
//			String uid = tokenArr[0];
	...
	public void sendLoginLog(UserToken userToken){
		try {
			LogSender sender = UdpLogSender.getInstace("192.168.4.200", 11100,new DatagramSocket());
	...
	public void sendPayLog(GameServer gameServer,PaymentOrder order,BigDecimal finishAmount,int gold,KaiYingTwPaymentObj cb){
		try {
			LogSender sender = UdpLogSender.getInstace("192.168.4.200", 11101,new DatagramSocket());
			LogEntryClient client = LogEntryClient.getInstace(sender);
	...
	public boolean doPayment(PaymentObj paymentObj) {
		...
		if (!KaiYingSASdk.instance().checkPayCallbackSign(cb)) {
		...
		if (order == null) {
			...
			//恺英海外汇款    手动创建订单
			TradeInfo info = createOrderInfo(getPatenerEnum().getPartenerId()+"", KaiYingSASdk.instance().getServerId(Integer.parseInt(cb.getSid())), cb.getUserId(), new BigDecimal(cb.getAmount()), "");
	...
	public PartenerEnum getPatenerEnum() {
		return PartenerEnum.KaiYingSAAndr;
	}
