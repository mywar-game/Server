...
import com.fantingame.game.gamecenter.partener.yxcq.baidu.BaiduPaymentObj;
import com.fantingame.game.gamecenter.partener.yxcq.baidu.YXCQBaiduSdk;
...
public class YXCQBaiduServiceImpl extends BasePartnerService {

	private static final Logger logger = Logger.getLogger(YXCQBaiduServiceImpl.class);
	@Autowired
	...
	public UserToken login(String token, String partnerId, String serverId, long timestamp, String sign, Map<String, String> params) {
		...
		try {
			account = YXCQBaiduSdk.instance().vertifySession(token);
			...
			if (account != null && account.containsKey("userId") && (Integer) account.get("userId") != 0) {
				...
				if (YXCQBaiduSdk.instance().vertifySign(userId, cmFlag, signTmp)) {
	...
	public boolean doPayment(PaymentObj paymentObj) {
		...
		if (!YXCQBaiduSdk.instance().checkPayCallbackSign(cb)) {
	...
	public PartenerEnum getPatenerEnum() {
		return PartenerEnum.YXCQ_BaiDu;
	}
