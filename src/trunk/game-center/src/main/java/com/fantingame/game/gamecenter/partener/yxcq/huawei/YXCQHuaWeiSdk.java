 ...
 * @author yezp
 */
public class YXCQHuaWeiSdk extends BaseSdk {
	private static final Logger logger = Logger.getLogger(YXCQHuaWeiSdk.class);

	private static YXCQHuaWeiSdk ins;
	private final static String PROTOCOL_HEAD = "https://";
	private static Properties prop;
	private String publicKey;
	private String host;

	public static YXCQHuaWeiSdk instance() {
		synchronized (YXCQHuaWeiSdk.class) {
			if (ins == null) {
				ins = new YXCQHuaWeiSdk();
			}
		}
		return ins;
	}

	private YXCQHuaWeiSdk() {
		loadSdkProperties();
	}
