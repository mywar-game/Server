...
public class GooglePlayController {
	private static Logger LOG = Logger.getLogger(GooglePlayController.class);
	...
	public ModelAndView payCallback(HttpServletRequest req, HttpServletResponse res) {
		PartnerService ps = serviceFactory.getBean(PartenerEnum.GooglePlay.getPartenerId());
		...
		try {
			if(StringUtils.isNotEmpty(json)
				...
				GooglePaymentObj data = new GooglePaymentObj();
				data.setJson(json);
				data.setSignature(signature);
				LOG.info("googlePayment:"+JSON.toJSONString(data));
