...
public class PPAssistantServiceImpl extends BasePartnerService {

	public static Logger logger = Logger.getLogger(PPAssistantServiceImpl.class);
	...
	public UserToken login(String token, String partnerId, String serverId, long timestamp, String sign, Map<String, String> params) {
		...
		try {
			String info = PPAssistantSdk.instance().verifySession(token);
	...
	public boolean doPayment(PaymentObj paymentObj) {
		...
		if (!PPAssistantSdk.instance().checkPayCallbackSign(cb)) {
	...
	public PartenerEnum getPatenerEnum() {
		return PartenerEnum.PpAssistant;
	}
}
