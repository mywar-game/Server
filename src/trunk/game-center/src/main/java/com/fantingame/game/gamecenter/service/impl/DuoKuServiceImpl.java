...
import com.fantingame.game.gamecenter.partener.duoku.DuokuPaymentObj;
...
public class DuoKuServiceImpl extends BasePartnerService {
	private static final Logger logger = Logger.getLogger(DuoKuServiceImpl.class);
	@Autowired
	...
	public UserToken login(String token, String partnerId, String serverId, long timestamp, String sign, Map<String, String> params) {
		...
		try {
			...
			if (DuoKuSdk.instance().verifySession(session, uid)) {
...
//		if (!DuoKuSdk.instance().checkPayCallbackSign(cb)) {
