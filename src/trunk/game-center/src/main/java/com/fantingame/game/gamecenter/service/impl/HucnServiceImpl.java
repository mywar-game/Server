...
public class HucnServiceImpl extends BasePartnerService {
	private static final Logger logger = Logger.getLogger(HucnServiceImpl.class);
	@Autowired
	...
	public boolean doPayment(PaymentObj paymentObj) {
		...
		// 更新订单状态
		if (this.paymentOrderDao.updateStatus(order.getOrderId(), OrderStatus.STATUS_FINISH, cb.getOrderid(), finishAmount, cb.getExtrainfo())) {
			GameServer gameServer = serverListDao.getServerByServerIdAndPartenerId(order.getServerId(), order.getPartnerId());
			// 请求游戏服，发放游戏货币
			if (!GameApiSdk.getInstance().doPayment(order.getPartnerId(), order.getServerId(), order.getPartnerUserId(), "", order.getOrderId(), finishAmount, gold, "", cb.getMsg(),gameServer)) {
				// 如果失败，要把订单置为未支付
				this.paymentOrderDao.updateStatus(order.getOrderId(), OrderStatus.STATUS_NOT_PAY, cb.getOrderid(), finishAmount, cb.getExtrainfo());
		...
		this.paymentOrderDao.updateStatus(order.getOrderId(), OrderStatus.STATUS_ERROR, cb.getOrderid(), finishAmount, cb.getMsg());
		logger.error("充值失败：" + Json.toJson(cb));
		return false;
	}
