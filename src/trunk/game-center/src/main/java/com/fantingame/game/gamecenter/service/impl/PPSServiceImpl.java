...
public class PPSServiceImpl extends BasePartnerService {
	private static final Logger logger = Logger.getLogger(PPSServiceImpl.class);
	@Autowired
	...
	public UserToken login(String token, String partnerId, String serverId, long timestamp, String sign, Map<String, String> params) {
		...
		try {
			logger.info("PPS login token : " + token);
			String[] tokenArr = token.split(":");
			String uid = tokenArr[0];
			String session = tokenArr[1];
			String times = tokenArr[2];
			if (PPSSdk.instance().verifySession(session, uid, times)) {
				...
				logger.info("PPS userToken=" + userToken);
				return userToken;
			}
	...
	public boolean doPayment(PaymentObj paymentObj) {
		...
		PPSPaymentObj cb = (PPSPaymentObj) paymentObj;
		if (!PPSSdk.instance().checkPayCallbackSign(cb)) {
			logger.error("签名不正确" + Json.toJson(paymentObj));
			return false;
		}
		logger.info("game id:" + cb.getUserData());
		PaymentOrder order = paymentOrderDao.get(cb.getUserData());
	...
	public String createOrder(String partnerId, String serverId, String partnerUserId, BigDecimal amount, String tradeName) {
		...
		info.setNotifyUrl("http://wapi.andr.hw.fantingame.com:80/webApi/ppsPayment.do");
//		info.setNotifyUrl("http://203.195.190.121:8089/webApi/ppsPayment.do");
	...
	public String createOrder(String partnerId, String serverId,
		...
		info.setNotifyUrl("http://wapi.andr.hw.fantingame.com:80/webApi/ppsPayment.do");
//		info.setNotifyUrl("http://203.195.190.121:8089/webApi/ppsPayment.do");
		logger.info("PPS orderInfo:" + info.getNotifyUrl());
