...
public class KuaiBoServiceImpl extends BasePartnerService {
	private static final Logger logger = Logger.getLogger(KuaiBoServiceImpl.class);
	@Autowired
	...
	public UserToken login(String token, String partnerId, String serverId, long timestamp, String sign, Map<String, String> params) {
		...
		try {
			...
			if (KuaiBoSdk.instance().verifySession(session, uid)) {
	...
	public boolean doPayment(PaymentObj paymentObj) {
		...
		KuaiBoPaymentObj cb = (KuaiBoPaymentObj) paymentObj;

		if (!KuaiBoSdk.instance().checkPayCallbackSign(cb)) {
			logger.error("签名不正确" + Json.toJson(paymentObj));
			return false;
		}
		logger.info("game id:" + cb.getNote());
		PaymentOrder order = paymentOrderDao.get(cb.getNote());
		...
		BigDecimal finishAmount = new BigDecimal(cb.getPaymentFee());
		if (order.getAmount().compareTo(finishAmount) != 0) {
			logger.error("订单金额不符：" + Json.toJson(cb));
			return false;
		}

		if (!cb.getPaymentStatusCode().equals("0")) {
