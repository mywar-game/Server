...
import com.fantingame.game.gamecenter.partener.lenovo.TransSyncTemp;
...
public class LenovoServiceImpl extends BasePartnerService {
	...
	public UserToken login(String token, String partnerId, String serverId, long timestamp, String sign, Map<String, String> params) {
		...
		try {
			...
			if (account != null && account.containsKey("accountId") && !StringUtils.isBlank(account.get("accountId"))) {
				...
				UserToken userToken = GameApiSdk.getInstance().loadUserToken(accountId, partnerId, serverId, "0",gameServer, params);
				return userToken;
			}
	...
	public String createOrder(String partnerId, String serverId, String partnerUserId, BigDecimal amount, String tradeName) {
		...
		logger.info("Lenovo createOrder tradeNameDecode = " + tradeNameDecode);
...
//		info.setNotifyUrl("http://203.195.190.121:8089/webApi/lenovoPayment.do");
		// 联想以分为单位
	...
	public String createOrder(String partnerId, String serverId, String partnerUserId, BigDecimal amount, String tradeName, String qn) {
		...
		info.setNotifyUrl(LenovoSdk.instance().getPayCallback());
//		info.setNotifyUrl("http://203.195.190.121:8089/webApi/lenovoPayment.do");
		logger.info("Lenovo orderInfo:" + info.getNotifyUrl());
		return Json.toJson(info);
	}
	
	@Override
	public PartenerEnum getPatenerEnum() {
		return PartenerEnum.Lenovo;
	}
}
